...
const PLACEHOLDER_IMAGE = 'https://placehold.co/400x300/e0e7ff/6366f1?text=Sin+imagen'
...
export function RoomCard({ room }: RoomCardProps) {
  ...
  const handleFavoriteClick = (e: React.MouseEvent) => {
    ...
    if (isFavorite) {
      removeFavorite.mutate(room.id)
    } else {
      addFavorite.mutate(room.id)
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
        ...
        >
          <svg
            className={`h-5 w-5 transition-colors ${isFavorite ? 'fill-red-500 text-red-500' : 'fill-none text-gray-500'}`}
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
          >
